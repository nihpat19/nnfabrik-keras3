...
from nnfabrik.templates.trained_model import TrainedModelBase, TrainedOptunaModelBase
...
# def optuna_checkpoint(nnfabrik):
#     conn_clone = clone_conn(dj.conn())
#     schema_clone = CustomSchema(nnfabrik.schema.database, connection=conn_clone)

#     @schema_clone
#     class OptunaCheckpoint(dj.Manual):
#         storage = "minio"
...
class TrainedOptunaModelChkptBase(TrainedOptunaModelBase):
    '''
     checkpointmodel for Optuna
    '''
    checkpoint_table = None  # assign the output of `my_checkpoint` here (i.e. `Checkpoint`)
    keys = [
        "model_fn",
